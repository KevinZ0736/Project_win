...
{
	if (m_ftpconn != 0) { FtpQuit(m_ftpconn); printf("b\n"); m_ftpconn = 0; }

	m_connectfailed = m_loginfailed = m_optionfailed = false;

	if (FtpConnect(host, &m_ftpconn) == false) { m_connectfailed = true; printf("b\n"); return false; }

	if (FtpLogin(username, password, m_ftpconn) == false) { m_loginfailed = true; printf("c\n"); return false; }

	if (FtpOptions(FTPLIB_CONNMODE, (long)imode, m_ftpconn) == false) { m_optionfailed = true; printf("d\n"); return false; }
