...
int main(int argc, char* argv[])
{
	...
	if ((strlen(starg.remotecols) == 0) || (strlen(starg.localcols) == 0))
	{
		...
		if (strlen(starg.remotecols) == 0)  strcpy(starg.remotecols, TABCOLS.m_allcols.c_str());
		if (strlen(starg.localcols) == 0)   strcpy(starg.localcols, TABCOLS.m_allcols.c_str());
	}
